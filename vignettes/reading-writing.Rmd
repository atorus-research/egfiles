...
While `eg_read()` and `eg_write()` handle raw file transfers, you probably spend most of your time working with data files that you want to load directly into R as data frames. egfiles provides a set of functions that handle the download-read or write-upload workflow in a single step.
...
Note: egfiles can write XPT files but not native SAS7BDAT files. If you need SAS7BDAT, you'll need to use SAS itself or another tool.
...
The format-specific functions require additional packages that aren't installed by default with egfiles:
...
If you try to use a function without the required package installed, egfiles will prompt you to install it:
...
egfiles uses the file extension to determine the format when downloading. Make sure your files have the correct extension:
